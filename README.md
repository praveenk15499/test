...
{   //die($rest."/".$cur_url);
    ...
    if($u)
    {
        $tk=str_replace('Bearer ','',$_SESSION['access_atlas']);
        if(!$tk){
         require_once realpath(dirname(__FILE__ ) . '/Auth_login.php');$object = new Auth_login();
         $tkn=str_replace('Bearer ','',$object->Authorization());
        }
         $url = Atlas_User_new.'users/'.$u.'/avatar?size=thumbnail&default=drama&access_token='.$tk; 
        #die($url);
        $FLS=file_get_contents($url);
        if($FLS) {
            $t="jpg";
            if(strpos($FLS,'PNG')!==false) $t="png";
